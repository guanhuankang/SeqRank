    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            for blk in block:
                x = blk(x, H, W)
            x = norm(x)
            out[key_names[i]] = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
...
class SMT_S(SMT, Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__(embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16], mlp_ratios=[4, 4, 4, 2], 
...
@BACKBONE_REGISTRY.register()
class SMT_B(SMT, Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__(embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16], mlp_ratios=[8, 6, 4, 2], 
...
@BACKBONE_REGISTRY.register()
class SMT_L(SMT, Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__(embed_dims=[96, 192, 384, 768], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16], mlp_ratios=[8, 6, 4, 2], 
        qkv_bias=True, depths=[4, 6, 28, 4], ca_attentions=[1, 1, 1, 0], head_conv=7, expand_ratio=2)
        self.default_cfg = _cfg()
...
if __name__ == '__main__':
    ...
    ### ptflops cal ###
    flops_count, params_count = get_model_complexity_info(model,(3,800,800), as_strings=True, print_per_layer_stat=False)
